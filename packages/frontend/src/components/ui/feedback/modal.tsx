...
const modalVariants = cva(
  ...
  "relative bg-[var(--card-bg)] rounded-3xl shadow-2xl w-full max-h-[90vh] overflow-hidden",
  {
    variants: {
      size: {
        sm: "max-w-md",
            ...
            <div className="flex items-start justify-between p-6 border-b border-[var(--border)]">
              <div className="flex-1 min-w-0">
                {title && (
                  <h2 
                    id="modal-title"
                    className="text-xl font-semibold text-[var(--heading-color)] mb-1"
                  >
                    {title}
                  </h2>
                )}
                {description && (
                  <p 
                    ...
                    className="text-sm text-[var(--body-color)]"
                  >
                    {description}
                  </p>
                )}
              ...
              {showCloseButton && (
                <button
                  onClick={onClose}
                  className="ml-4 p-2 text-[var(--muted)] hover:text-[var(--foreground)] hover:bg-[var(--hover-overlay)] rounded-lg transition-colors"
...
const ModalHeader = React.forwardRef<HTMLDivElement, ModalHeaderProps>(
  ({ title, description, actions, icon, className, children, ...props }, ref) => (
    <div 
      ref={ref}
      className={cn("flex items-start justify-between p-6 border-b border-[var(--border)]", className)}
      {...props}
    >
      <div className="flex items-start space-x-3 flex-1 min-w-0">
        {icon && (
          <div className="flex-shrink-0 p-2 rounded-lg bg-[var(--primary)]/10">
        ...
        <div className="flex-1 min-w-0">
          {title && (
            <h2 className="text-xl font-semibold text-[var(--heading-color)] mb-1">
              {title}
            </h2>
          )}
          {description && (
  ...
  ({ justify = 'end', spacing = 'md', className, ...props }, ref) => {
    ...
    return (
      <div 
        ref={ref}
        className={cn(
          "flex items-center p-6 border-t border-[var(--border)] bg-[var(--background-secondary)]",
      ...
      >
        ...
        <ModalFooter>
          <button
            ...
            disabled={loading}
            className="px-4 py-2 text-sm font-medium text-[var(--foreground-secondary)] bg-[var(--background)] border border-[var(--border)] rounded-lg hover:bg-[var(--background-secondary)] disabled:opacity-50 transition-colors"
          >
            {cancelText}
          </button>
          ...
          <button
            ...
            className={cn(
              ...
              variant === 'default' && "bg-[var(--primary)] hover:bg-[var(--primary-dark)]"
            )}
          >
            {loading && (
...
const FormModal = React.forwardRef<HTMLDivElement, FormModalProps>(
  ({
    ...
    const submitVariantClasses = {
      primary: "bg-[var(--primary)] hover:bg-[var(--primary-dark)]",
