...
import { BrandProfileCoreService, BrandProfile } from '../brands/core/brandProfile.service';

export class BrandProfileService extends BrandProfileCoreService {
  ...
  async getConnectionOpportunities(
    ...
    try {
      ...
      for (const opportunity of opportunities) {
        const compatibility = await this.calculateCompatibilityScore(businessId, opportunity._id.toString());

        results.push({
          ...opportunity.toObject(),
          compatibilityScore: compatibility.score,
          connectionReason: this.getConnectionReason(business, opportunity.toObject(), manufacturerId),
  ...
  async getBrandProfileForManufacturer(brandId: string, manufacturerId?: string): Promise<any> {
    try {
      const brand = await Business.findById(brandId).select('-password');
    ...
    try {
      ...
      return {
        ...
        applied: {
          ...
          hasFilters: Boolean(filterOptions?.industry || filterOptions?.verified !== undefined || filterOptions?.search)
        }
      };
    } catch (error) {
...
export type { BrandProfile } from '../brands/core/brandProfile.service';
