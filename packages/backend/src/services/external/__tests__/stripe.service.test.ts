  ...
  describe('Checkout Sessions', () => {
    describe('createCheckoutSession', () => {
      ...
      it('should forward metadata when provided', async () => {
        const customerId = 'cus_test123';
        const plan = 'foundation';
        const metadata = { plan, campaign: 'fall' };
        const mockSession = {
          id: 'cs_meta_123',
          url: 'https://checkout.stripe.com/pay/cs_meta_123'
        };

        mockStripeInstance.checkout.sessions.create.mockResolvedValue(mockSession);

        const result = await stripeService.createCheckoutSession(customerId, plan, { metadata });

        expect(result).toBe('cs_meta_123');
        expect(mockStripeInstance.checkout.sessions.create).toHaveBeenCalledWith({
          customer: customerId,
          mode: 'subscription',
          line_items: [{ price: 'price_1RqIhPAUE6HtX6lzNr05UfW6', quantity: 1 }],
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid plan keys', async () => {
      ...
      await expect(stripeService.createSubscription(customerId, invalidPlan))
        .rejects.toThrow('Stripe price ID is not configured for plan "invalid_plan"');
    });
  });
