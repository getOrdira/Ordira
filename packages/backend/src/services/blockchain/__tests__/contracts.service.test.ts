...
describe('BlockchainContractsService', () => {
  ...
  beforeEach(() => {
    ...
    const mockContract = {
      ...
      transfer: jest.fn().mockResolvedValue(BigInt(100000)),
    ...
    // Mock the createAppError function to throw the error instead of returning it
    (createAppError as any).mockImplementation((message: string, statusCode: number, code?: string) => {
      const error = new Error(message) as any;
      error.statusCode = statusCode;
      error.code = code;
      throw error;
    ...
    describe('getTokenBalance', () => {
      ...
      it('should handle missing token contract address', async () => {
        ...
        await expect(BlockchainContractsService.getTokenBalance(mockAddress))
          .rejects.toMatchObject({
            statusCode: 500,
            message: expect.stringContaining('TOKEN_CONTRACT_ADDRESS environment variable not configured')
          });
      });
    ...
    describe('waitForTransaction', () => {
      ...
      it('should handle transaction not found', async () => {
        ...
        await expect(BlockchainContractsService.waitForTransaction(mockTxHash))
          .rejects.toMatchObject({
            statusCode: 500,
            message: expect.stringContaining('Failed to wait for transaction')
          });
      });
    });
    ...
    describe('getOptimalGasPrice', () => {
      ...
      it('should validate priority parameter', async () => {
        await expect(BlockchainContractsService.getOptimalGasPrice('invalid' as any))
          .rejects.toMatchObject({
            statusCode: 500,
            message: expect.stringContaining('Failed to get optimal gas price')
          });
      });
    ...
    describe('estimateGas', () => {
      ...
      it('should validate parameters', async () => {
        await expect(BlockchainContractsService.estimateGas('', [], 'transfer', []))
          .rejects.toMatchObject({
            statusCode: 400,
            message: expect.stringContaining('Invalid contractAddress format')
          });
    ...
    describe('batchCall', () => {
      ...
      it('should validate individual calls', async () => {
        ...
        await expect(BlockchainContractsService.batchCall(calls))
          .rejects.toMatchObject({
            statusCode: 400,
            message: expect.stringContaining('Invalid calls[0].contractAddress format')
          });
      });
