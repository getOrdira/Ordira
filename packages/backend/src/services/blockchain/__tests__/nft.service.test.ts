...
// StorageService is mocked at the module level
...
describe('NftService', () => {
  ...
  beforeEach(() => {
    ...
    (MockedCertificate.create as jest.Mock).mockResolvedValue(mockCertificate);
    
    // Mock findOne with lean method
    ...
    MockedCertificate.findOne.mockReturnValue(mockFindOneQuery as any);
    ...
    MockedCertificate.find.mockReturnValue(mockQuery as any);
  });
    ...
    describe('mintNft', () => {
      ...
      it('should handle minting errors', async () => {
        ...
        await expect(nftService.mintNft(mockBusinessId, params))
          .rejects.toMatchObject({
            statusCode: 500,
            message: expect.stringContaining('Failed to mint NFT')
          });
      });
    });
  });
    ...
    describe('getContractMetadata', () => {
      ...
      it('should handle contract not found', async () => {
        MockedBlockchainProviderService.getReadOnlyContract.mockReturnValue({
          ...
          symbol: jest.fn().mockRejectedValue(new Error('Contract not found')),
          totalSupply: jest.fn().mockRejectedValue(new Error('Contract not found')),
          owner: jest.fn().mockRejectedValue(new Error('Contract not found'))
        } as any);

        await expect(nftService.getContractMetadata(mockContractAddress, mockBusinessId))
          .rejects.toMatchObject({
            statusCode: 500,
            message: expect.stringContaining('Failed to get contract metadata')
          });
      });
    });
    ...
    describe('getTokenURI', () => {
      ...
      it('should handle token not found', async () => {
        ...
        await expect(nftService.getTokenURI(mockContractAddress, mockTokenId))
          .rejects.toMatchObject({
            statusCode: 500,
            message: expect.stringContaining('Failed to get token URI')
          });
      });
    });
    ...
    describe('getTokenOwner', () => {
      ...
      it('should handle token not found', async () => {
        ...
        await expect(nftService.getTokenOwner(mockContractAddress, mockTokenId))
          .rejects.toMatchObject({
            statusCode: 500,
            message: expect.stringContaining('Failed to get token owner')
          });
      });
    });
  });
