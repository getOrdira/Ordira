    ...
    try {
      ...
      if (!workspace.enabledFeatures.fileSharing) {
        throw {
          ...
          message: 'File sharing feature is not enabled for this workspace. Brand requires Growth plan or higher, and Manufacturer requires Professional plan or higher.'
        };
      }
      ...
      const file = await FileAttachment.create({
        ...
        fileType: input.mimeType, // Schema uses fileType for MIME type
        s3Bucket: input.s3Bucket || process.env.AWS_S3_BUCKET || 'ordira-uploads',
        s3Key: input.s3Key,
        s3Url: input.s3Url,
        s3Region: input.s3Region || process.env.AWS_REGION || 'us-east-1',
  ...
  public async createNewVersion(
    ...
    try {
      ...
      const newVersion = await FileAttachment.create({
        ...
        fileSize: input.fileSize,
        fileType: input.mimeType, // Schema uses fileType for MIME type
        s3Bucket: input.s3Bucket || parentFile.s3Bucket,
        s3Key: input.s3Key,
        s3Url: input.s3Url,
        s3Region: input.s3Region || parentFile.s3Region,
        description: input.description,
