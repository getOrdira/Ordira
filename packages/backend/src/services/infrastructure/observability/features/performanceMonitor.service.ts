...
import { PerformanceMetrics, PerformanceSystemHealth } from '../utils/types';
...
export class PerformanceService {
   ...
   */
  async getSystemHealth(): Promise<PerformanceSystemHealth> {
    const memUsage = process.memoryUsage();
    const cpuUsage = process.cpuUsage();
    const uptime = Date.now() - this.startTime;

    // Database health
    const dbHealth = await databaseServices.databaseService.healthCheck();
    
    // Cache health
    const cacheHealth = await cacheServices.cacheStoreService.healthCheck();
    const cacheStats = await cacheServices.cacheStoreService.getStats();
  ...
  async optimizePerformance(): Promise<{
    ...
    try {
      await databaseServices.databaseService.optimizeIndexes();
