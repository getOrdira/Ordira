// src/models/certificates/certificate.model.ts
...
CertificateSchema.methods.executeTransfer = async function(): Promise<boolean> {
  ...
  try {
    ...
    await notificationsService.sendEmail(this.recipient, 'Certificate Transfer Success', `Your certificate has been successfully transferred to the brand wallet ${this.brandWallet}.`);
    ...
    const { notificationsService } = await import('../../services/notifications/notifications.service');
    await notificationsService.sendEmail(this.recipient, 'Certificate Transfer Failure', `Your certificate has been failed to transfer to the brand wallet ${this.brandWallet}.`);
